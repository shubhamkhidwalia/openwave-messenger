...
const App = (() => {
  ...
  let notifCounts={},forwardMsg=null,activeReactionMsgId=null;
  ...
  async function init() {
    UI.loadTheme();
    try{const cfg=await fetch('/api/config').then(r=>r.json());if(cfg.bot_username){const l=document.getElementById('bot-link');if(l)l.href=`https://t.me/${cfg.bot_username}`;document.getElementById('invite-banner').style.display='block';}}catch{}
    const inviteMatch=location.pathname.match(/^\/invite\/([a-zA-Z0-9]+)$/);
    if(inviteMatch){const code=inviteMatch[1];history.replaceState({},'','/');const tok=localStorage.getItem('ow_token');if(!tok){showAuth('register',code);return;}}
    const tok=localStorage.getItem('ow_token');
    if(tok){try{const d=await API.me();currentUser=d.user;startApp();}catch(err){if(err.isNetworkError)showNetworkError();else{localStorage.removeItem('ow_token');showAuth();}}}
    else showAuth();
  }
  ...
  function showNetworkError(){
    ...
    document.querySelector('.auth-card').innerHTML=`<span class="auth-logo">🌊</span><h1 class="auth-title">OpenWave</h1><p class="auth-sub" style="color:var(--danger)">No connection</p><p style="font-size:14px;color:var(--muted);margin-bottom:1.5rem;line-height:1.6">Could not reach the server. Check your internet and try again.</p><button class="btn-primary" onclick="location.reload()">Try Again</button>`;
  }
  ...
  function showAuth(tab='login',prefillInvite=''){
    ...
    document.getElementById('app').classList.add('hidden');
    setupAuthHandlers();
    if(tab==='register'||prefillInvite){document.querySelectorAll('.auth-tab').forEach(t=>t.classList.remove('active'));document.querySelectorAll('.auth-form').forEach(f=>f.classList.remove('active'));document.querySelector('.auth-tab[data-tab="register"]').classList.add('active');document.getElementById('register-form').classList.add('active');document.getElementById('invite-banner').style.display='block';}
    if(prefillInvite)document.getElementById('reg-invite').value=prefillInvite;
  }

  function setupAuthHandlers(){
    document.querySelectorAll('.auth-tab').forEach(tab=>{
      tab.onclick=()=>{document.querySelectorAll('.auth-tab').forEach(t=>t.classList.remove('active'));document.querySelectorAll('.auth-form').forEach(f=>f.classList.remove('active'));tab.classList.add('active');document.getElementById(tab.dataset.tab+'-form').classList.add('active');const b=document.getElementById('invite-banner');if(b)b.style.display=tab.dataset.tab==='register'?'block':'none';};
    });
    const lf=document.getElementById('login-form');
    if(!lf._b){lf._b=true;lf.addEventListener('submit',async e=>{e.preventDefault();const btn=e.target.querySelector('button');btn.disabled=true;btn.textContent='Signing in…';try{const d=await API.login({username:document.getElementById('login-username').value,password:document.getElementById('login-password').value});localStorage.setItem('ow_token',d.token);currentUser=d.user;startApp();}catch(err){document.getElementById('login-error').textContent=err.message;btn.disabled=false;btn.textContent='Sign In';}});}
    const rf=document.getElementById('register-form');
    if(!rf._b){rf._b=true;rf.addEventListener('submit',async e=>{e.preventDefault();const btn=e.target.querySelector('button');btn.disabled=true;btn.textContent='Creating…';try{if(typeof checkCaptcha==='function'&&!checkCaptcha()){document.getElementById('reg-error').textContent='Wrong answer — try again';if(typeof refreshCaptcha==='function')refreshCaptcha();btn.disabled=false;btn.textContent='Create Account';return;}const d=await API.register({username:document.getElementById('reg-username').value,display_name:document.getElementById('reg-name').value,phone:document.getElementById('reg-phone').value||undefined,password:document.getElementById('reg-password').value,invite_code:document.getElementById('reg-invite').value.trim()});localStorage.setItem('ow_token',d.token);currentUser=d.user;startApp();}catch(err){document.getElementById('reg-error').textContent=err.message;btn.disabled=false;btn.textContent='Create Account';}});}
  }
  ...
  function updateMenuProfile(){
    ...
    const mp=document.getElementById('menu-phone');if(mp)mp.textContent=currentUser.phone||'';
  }
  ...
  function setupEventListeners(){
    ...
    document.getElementById('starred-btn')?.addEventListener('click',showStarred);
    ...
    document.getElementById('chat-search-btn')?.addEventListener('click',()=>{const b=document.getElementById('chat-search-bar');b.classList.toggle('open');if(b.classList.contains('open'))document.getElementById('chat-search-input').focus();else closeChatSearch();});
  ...
  function setupSwipeBack(){
    ...
    ca.addEventListener('touchmove',e=>{if(!activeChatId)return;const dx=e.touches[0].clientX-sx,dy=Math.abs(e.touches[0].clientY-sy);if(dx>20&&dy<80&&sx<50){dragging=true;const c=Math.min(dx,window.innerWidth);ca.style.transform=`translateX(${c}px)`;ca.style.transition='none';}},{passive:true});
  ...
  function setupVoiceRecorder(){
    ...
    const start=async()=>{if(!navigator.mediaDevices){UI.toast('Microphone not available');return;}try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});recordChunks=[];recordSeconds=0;mediaRecorder=new MediaRecorder(stream);mediaRecorder.ondataavailable=e=>recordChunks.push(e.data);mediaRecorder.onstop=()=>{stream.getTracks().forEach(t=>t.stop());sendVoiceNote();};mediaRecorder.start();btn.classList.add('recording');timerEl.classList.add('visible');recordTimer=setInterval(()=>{recordSeconds++;timerEl.textContent=`${Math.floor(recordSeconds/60)}:${String(recordSeconds%60).padStart(2,'0')}`;},1000);}catch{UI.toast('Microphone access denied');}};
    const stop=()=>{if(mediaRecorder&&mediaRecorder.state==='recording')mediaRecorder.stop();btn.classList.remove('recording');timerEl.classList.remove('visible');clearInterval(recordTimer);recordSeconds=0;timerEl.textContent='0:00';};
  ...
  async function sendVoiceNote(){
    ...
    try{const data=await API.upload(file);const dur=recordSeconds||1;const res=await API.sendMessage(activeChatId,{type:'file',content:JSON.stringify({url:data.url,name:'Voice message',size:blob.size,voice:true,duration:dur})});if(!messages[activeChatId])messages[activeChatId]=[];messages[activeChatId].push(res.message);appendMessage(res.message);}
  ...
  function setupWSHandlers(){
    ...
    WS.on('message_edited',data=>{const{message,chat_id}=data;if(messages[chat_id]){const i=messages[chat_id].findIndex(m=>m.id===message.id);if(i!==-1)messages[chat_id][i]=message;}if(chat_id===activeChatId){const el=document.querySelector(`[data-msg-id="${message.id}"] .bubble-text`);if(el){el.innerHTML=UI.linkify(message.content);el.closest('.bubble').classList.add('edited');}}});
    ...
    el.innerHTML=chats.map(chat=>{
      ...
      return `<div class="chat-item ${chat.id===activeChatId?'active':''}" data-chat-id="${chat.id}" data-user-id="${chat.peer?.id||''}" onclick="App.openChat('${chat.id}')">
        ...
        <div class="chat-item-body">
          <div class="chat-item-top"><div class="chat-item-name">${UI.esc(name)}${isDisappearing?' ⏱':''}</div><div class="chat-item-time">${chat.last_msg_ts?UI.fmtDate(chat.last_msg_ts):''}</div></div>
  ...
  function updateChatPreview(chatId,msg){const c=chats.find(c=>c.id===chatId);if(!c)return;c.last_message=msg.content;c.last_msg_ts=msg.created_at;chats=[c,...chats.filter(c=>c.id!==chatId)];renderChatList();}
  ...
  async function openChat(chatId){
    ...
    ha.style.background=UI.colorForName(name);ha.innerHTML=avatarSrc?`<img src="${avatarSrc}" style="width:100%;height:100%;object-fit:cover">`:UI.initials(name);
    ...
    if(expiry){eb.classList.remove('hidden');et.textContent=`Disappearing messages: ${fmtExpiry(expiry)}`;}else eb.classList.add('hidden');
  ...
  async function loadMessages(chatId){try{const d=await API.getMessages(chatId,50,0);messages[chatId]=d.messages;renderMessages(chatId);}catch{UI.toast('Failed to load messages');}}
  ...
  function buildMsgHTML(msg,isMine,showAvatar,chatId){
    ...
    else if(msg.type==='file'){try{const f=JSON.parse(msg.content);if(f.voice){const bars=Array.from({length:20},()=>`<div class="voice-bar" style="height:${Math.max(4,Math.random()*22)}px"></div>`).join('');content=`<div class="voice-msg"><button class="voice-play" onclick="playVoice(this,'${f.url}')">▶</button><div class="voice-wave">${bars}</div><span class="voice-duration">${fmtDur(f.duration||0)}</span></div>`;}else{content=`<a href="${UI.esc(f.url)}" target="_blank" style="display:flex;align-items:center;gap:8px;color:inherit;text-decoration:none"><span style="font-size:22px">📎</span><span style="font-size:13px;line-height:1.4">${UI.esc(f.name)}<br><span style="opacity:.6;font-size:11px">${(f.size/1024).toFixed(1)} KB</span></span></a>`;}}catch{content=UI.linkify(msg.content);}}
    ...
    const statusIcon=isMine?`<span class="msg-status ${msg.delivered?'read':''}">${msg.delivered?'✓✓':'✓'}</span>`:'';
    return `<div class="msg-row ${isMine?'outgoing':'incoming'}" data-msg-id="${msg.id}">
      ...
      <div><div class="bubble ${msg.edited?'edited':''} ${isStarred?'starred':''} ${isExpiring?'expiring':''}">
      ...
      </div>${reactionsHTML}</div></div>`;
  }

  function appendMessage(msg){
    const el=document.getElementById('messages-list');el.querySelector('[style*="No messages"]')?.remove();
  ...
  async function sendFile(file){
    ...
    try{const d=await API.upload(file);const isImage=file.type.startsWith('image/');const res=await API.sendMessage(activeChatId,{type:isImage?'image':'file',content:isImage?d.url:JSON.stringify({url:d.url,name:d.name,size:d.size})});if(!messages[activeChatId])messages[activeChatId]=[];messages[activeChatId].push(res.message);appendMessage(res.message);UI.toast('Sent ✓');}
  ...
  function toggleDisappearing(){
    ...
    const options=[{label:'Off',value:0},{label:'1 day',value:86400},{label:'1 week',value:604800},{label:'1 month',value:2592000}];
    ...
    picker.innerHTML=`<div style="background:var(--card);border-radius:24px 24px 0 0;width:100%;max-width:480px;padding-bottom:env(safe-area-inset-bottom)"><div style="width:40px;height:4px;background:var(--border);border-radius:2px;margin:12px auto 0"></div><div style="padding:16px 20px;border-bottom:1px solid var(--border)"><div style="font-size:18px;font-weight:800;color:var(--text)">⏱ Disappearing Messages</div><div style="font-size:13.5px;color:var(--muted);margin-top:4px">Messages auto-delete after being sent. Only you see this setting.</div></div>${options.map(o=>`<div onclick="App.setDisappearing(${o.value})" style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;cursor:pointer;border-bottom:.5px solid var(--border-sub);font-size:15px;color:${o.value===current?'var(--accent)':'var(--text)'};font-weight:${o.value===current?700:500}"><span>${o.label}</span>${o.value===current?'<span style="color:var(--accent);font-size:20px">✓</span>':''}</div>`).join('')}<div onclick="this.parentElement.parentElement.remove()" style="padding:16px 20px;text-align:center;color:var(--muted);font-size:15px;cursor:pointer">Cancel</div></div>`;
    picker.onclick=e=>{if(e.target===picker)picker.remove();};document.body.appendChild(picker);
  }
  ...
  function setDisappearing(seconds){
    ...
    if(seconds===0)delete disappearingChats[activeChatId];else disappearingChats[activeChatId]=seconds;
    localStorage.setItem('ow_disappearing',JSON.stringify(disappearingChats));
    const eb=document.getElementById('expiry-banner'),et=document.getElementById('expiry-text');
    if(seconds){eb.classList.remove('hidden');et.textContent=`Disappearing messages: ${fmtExpiry(seconds)}`;}else eb.classList.add('hidden');
    renderChatList();UI.toast(seconds?`⏱ ${fmtExpiry(seconds)}`:'Disappearing messages off');
  }

  function fmtExpiry(s){return s>=2592000?'1 month':s>=604800?'1 week':s>=86400?'1 day':'Custom';}
  ...
  function addReaction(emoji,msgId){
    ...
    if(!users.length)delete reactions[id][emoji];if(!Object.keys(reactions[id]).length)delete reactions[id];
  ...
  function toggleStar(msgId){
    const i=starredMessages.indexOf(msgId);if(i!==-1)starredMessages.splice(i,1);else starredMessages.push(msgId);
    localStorage.setItem('ow_starred',JSON.stringify(starredMessages));
    const b=document.querySelector(`[data-msg-id="${msgId}"] .bubble`);if(b)b.classList.toggle('starred',starredMessages.includes(msgId));
  ...
  function showStarred(){
    ...
    el.innerHTML=starred.map(m=>{const chat=chats.find(c=>c.id===m.chat_id);return`<div class="starred-item" onclick="App.jumpToMessage('${m.chat_id}','${m.id}')"><div class="starred-item-chat">📍 ${UI.esc(getChatName(chat)||'')}</div><div class="starred-item-text">${UI.esc(m.content?.substring(0,120)||'')}</div><div class="starred-item-time">${UI.fmtDate(m.created_at)}</div></div>`;}).join('');
  }

  async function jumpToMessage(chatId,msgId){
    closeModal('modal-starred');if(chatId!==activeChatId)await openChat(chatId);
  ...
  function searchInChat(q){
    const msgs=messages[activeChatId]||[];document.querySelectorAll('.msg-highlight').forEach(e=>e.classList.remove('msg-highlight'));
    if(!q.trim()){document.getElementById('search-count').textContent='';return;}
    const results=msgs.filter(m=>m.content?.toLowerCase().includes(q.toLowerCase())&&m.type==='text');
    document.getElementById('search-count').textContent=results.length?`${results.length} result${results.length!==1?'s':''}`:' No results';
    if(results.length){const m=results[results.length-1];const el=document.querySelector(`[data-msg-id="${m.id}"]`);if(el){el.scrollIntoView({behavior:'smooth',block:'center'});el.classList.add('msg-highlight');}}
  }
  ...
  function showForward(msg){
    ...
    el.innerHTML=chats.filter(c=>c.id!==activeChatId).map(c=>{const name=getChatName(c);return`<div class="user-item" onclick="App.forwardTo('${c.id}')"><div class="user-item-avatar" style="background:${UI.colorForName(name)}">${UI.initials(name)}</div><div><div class="user-item-name">${UI.esc(name)}</div></div></div>`;}).join('')||`<div style="padding:2rem;text-align:center;color:var(--muted)">No other chats</div>`;
  }
  async function forwardTo(chatId){
    ...
    catch{UI.toast('Forward failed');}forwardMsg=null;
  }
  ...
  function showMsgContext(e,msg){
    ...
    const rect=menu.getBoundingClientRect();if(rect.right>window.innerWidth)menu.style.left=(e.clientX-rect.width)+'px';if(rect.bottom>window.innerHeight)menu.style.top=(e.clientY-rect.height)+'px';
  }
  ...
  function notifyMessage(msg,chat){
    if(!chat||msg.sender_id===currentUser?.id)return;if(msg.chat_id===activeChatId&&!document.hidden)return;
    if(Notification.permission==='default'){Notification.requestPermission();return;}if(Notification.permission!=='granted')return;
    notifCounts[chat.id]=(notifCounts[chat.id]||0)+1;const count=notifCounts[chat.id];
  ...
  async function createGroup(){
    const name=document.getElementById('group-name-input').value.trim();if(!name){UI.toast('Enter a group name');return;}
  ...
  async function showChatInfo(){
    ...
    if(chat.type==='direct'&&chat.peer){
      ...
      sheet.innerHTML=`<div class="profile-hero"><div class="profile-avatar-wrap"><div class="profile-avatar" style="background:${UI.colorForName(u.display_name)}">${u.avatar?`<img src="${u.avatar}" style="width:100%;height:100%;object-fit:cover">`:UI.initials(u.display_name)}</div></div><div class="profile-name-display">${UI.esc(u.display_name)}</div><div class="profile-username-display">@${UI.esc(u.username)}</div>${onlineUsers.has(u.id)?'<div style="color:var(--online);font-size:13px;margin-top:6px;font-weight:600">● Online</div>':''}</div>
      <div class="profile-sections"><div class="profile-section"><div class="profile-section-title">About</div><div class="profile-row"><span class="profile-row-icon">💬</span><div class="profile-row-body"><div class="profile-row-label">Bio</div><div class="profile-row-value">${UI.esc(u.bio||'No bio set')}</div></div></div>${u.phone?`<div class="profile-row"><span class="profile-row-icon">📱</span><div class="profile-row-body"><div class="profile-row-label">Phone</div><div class="profile-row-value">${UI.esc(u.phone)}</div></div></div>`:''}</div>
      ...
      sheet.innerHTML=`<div class="profile-hero"><div class="profile-avatar-wrap"><div class="profile-avatar" id="group-avatar-btn" style="background:${UI.colorForName(chat.name||'')};cursor:${isOwner?'pointer':'default'}">${chat.avatar?`<img src="${chat.avatar}" style="width:100%;height:100%;object-fit:cover">`:UI.initials(chat.name||'?')}</div>${isOwner?'<div class="profile-cam-btn" onclick="document.getElementById(\'g-avatar-input\').click()">📷</div><input type="file" id="g-avatar-input" hidden accept="image/*">':''}</div><div class="profile-name-display" id="group-name-display">${UI.esc(chat.name)}</div><div class="profile-username-display">${members.length} members</div></div>
      <div class="profile-sections">${isOwner?`<div class="profile-section"><div class="profile-section-title">Group Settings</div><div class="profile-row" onclick="App.editGroupName('${chat.id}')"><span class="profile-row-icon">✏️</span><div class="profile-row-body"><div class="profile-row-label">Group name</div><div class="profile-row-value editable">${UI.esc(chat.name)}</div></div><span class="profile-row-chevron">›</span></div></div>`:''}
      ...
      if(isOwner){document.getElementById('g-avatar-input')?.addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;try{const d=await API.upload(f);await API.updateChat(chat.id,{avatar:d.url});document.getElementById('group-avatar-btn').innerHTML=`<img src="${d.url}" style="width:100%;height:100%;object-fit:cover">`;const c=chats.find(x=>x.id===chat.id);if(c){c.avatar=d.url;renderChatList();document.getElementById('chat-header-avatar').innerHTML=`<img src="${d.url}" style="width:100%;height:100%;object-fit:cover">`;}UI.toast('Group photo updated ✓');}catch{UI.toast('Upload failed');}});}
    }
  }

  async function editGroupName(chatId){const chat=chats.find(c=>c.id===chatId);if(!chat)return;const newName=prompt('Group name:',chat.name);if(!newName||newName===chat.name)return;try{await API.updateChat(chatId,{name:newName});chat.name=newName;document.getElementById('group-name-display').textContent=newName;document.getElementById('chat-header-name').textContent=newName;renderChatList();UI.toast('Group renamed ✓');}catch{UI.toast('Failed to rename');}}
  ...
  function showSettings(){
    ...
    sc.innerHTML=`
      ...
      <div class="settings-group"><div class="settings-group-label">General</div>
        <div class="settings-item" onclick="App.showEditProfile()"><div class="settings-item-icon" style="background:rgba(124,58,237,.12)">👤</div><div class="settings-item-body"><div class="settings-item-label">Profile</div><div class="settings-item-sub">Name, photo, bio</div></div><span class="settings-item-right">›</span></div>
        <div class="settings-item"><div class="settings-item-icon" style="background:rgba(245,158,11,.12)">${isDark?'🌙':'☀️'}</div><div class="settings-item-body"><div class="settings-item-label">Dark Mode</div><div class="settings-item-sub">Currently ${isDark?'on':'off'}</div></div><div class="ow-toggle ${isDark?'active':''}" id="dark-toggle" onclick="event.stopPropagation();App.toggleThemeSettings()"></div></div>
      </div>
      <div class="settings-group"><div class="settings-group-label">Account</div>
        <div class="settings-item"><div class="settings-item-icon" style="background:rgba(16,185,129,.12)">🔑</div><div class="settings-item-body"><div class="settings-item-label">Username</div><div class="settings-item-sub">@${UI.esc(currentUser.username)}</div></div></div>
        ${currentUser.phone?`<div class="settings-item"><div class="settings-item-icon" style="background:rgba(16,185,129,.12)">📱</div><div class="settings-item-body"><div class="settings-item-label">Phone</div><div class="settings-item-sub">${UI.esc(currentUser.phone)}</div></div></div>`:''}
        <div class="settings-item" onclick="App.showEditField('display_name','Display Name',${JSON.stringify(currentUser.display_name)})"><div class="settings-item-icon" style="background:rgba(124,58,237,.12)">✏️</div><div class="settings-item-body"><div class="settings-item-label">Change Display Name</div><div class="settings-item-sub">${UI.esc(currentUser.display_name)}</div></div><span class="settings-item-right">›</span></div>
        <div class="settings-item" onclick="App.showEditField('bio','Bio',${JSON.stringify(currentUser.bio||'')})"><div class="settings-item-icon" style="background:rgba(124,58,237,.12)">💬</div><div class="settings-item-body"><div class="settings-item-label">About / Bio</div><div class="settings-item-sub">${UI.esc(currentUser.bio||'Add a bio…')}</div></div><span class="settings-item-right">›</span></div>
      </div>
      <div class="settings-group"><div class="settings-group-label">Privacy</div>
        <div class="settings-item"><div class="settings-item-icon" style="background:rgba(239,68,68,.12)">🔒</div><div class="settings-item-body"><div class="settings-item-label">Invite-only Registration</div><div class="settings-item-sub">Only people you invite can join</div></div><span class="settings-item-right" style="color:var(--online);font-size:12px;font-weight:700">ACTIVE</span></div>
        <div class="settings-item"><div class="settings-item-icon" style="background:rgba(239,68,68,.12)">⏱️</div><div class="settings-item-body"><div class="settings-item-label">Disappearing Messages</div><div class="settings-item-sub">Set per conversation in chat menu</div></div></div>
      </div>
      <div class="settings-group"><div class="settings-group-label">About</div>
        <div class="settings-item" onclick="window.open('https://github.com/shubhamkhidwalia/openwave-messenger','_blank')"><div class="settings-item-icon" style="background:rgba(0,0,0,.08)">📦</div><div class="settings-item-body"><div class="settings-item-label">GitHub Repository</div><div class="settings-item-sub">View source code, report issues</div></div><span class="settings-item-right">›</span></div>
        <div class="settings-item"><div class="settings-item-icon" style="background:rgba(245,158,11,.12)">👨‍💻</div><div class="settings-item-body"><div class="settings-item-label">Developed by</div><div class="settings-item-sub">Shubham Khidwalia — Open Source</div></div></div>
        <div class="settings-item"><div class="settings-item-icon">🌊</div><div class="settings-item-body"><div class="settings-item-label">Version</div><div class="settings-item-sub">OpenWave 1.0.0</div></div></div>
      </div>
      <div class="settings-group" style="margin-bottom:60px">
        <div class="settings-item" onclick="App.logout()" style="color:var(--danger)"><div class="settings-item-icon" style="background:rgba(239,68,68,.12)">🚪</div><div class="settings-item-body"><div class="settings-item-label" style="color:var(--danger)">Sign Out</div></div></div>
      </div>`;
  }

  function showEditProfile(){
    const u=currentUser;const overlay=document.createElement('div');overlay.className='edit-field-overlay';
    overlay.innerHTML=`<div class="edit-field-box"><div style="text-align:center;margin-bottom:20px"><div id="ep-avatar" style="width:80px;height:80px;border-radius:50%;background:${UI.colorForName(u.display_name)};display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:800;color:#fff;margin:0 auto 8px;overflow:hidden;cursor:pointer;box-shadow:0 4px 16px var(--accent-glow)" onclick="document.getElementById('ep-file').click()">${u.avatar?`<img src="${u.avatar}" style="width:100%;height:100%;object-fit:cover">`:UI.initials(u.display_name)}</div><input type="file" id="ep-file" hidden accept="image/*"><div style="font-size:13px;color:var(--accent);font-weight:600">Tap to change photo</div></div><div class="edit-field-title">Edit Profile</div><label style="font-size:11px;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:.08em;display:block;margin-bottom:5px">Display Name</label><input class="edit-field-input" id="ep-name" value="${UI.esc(u.display_name)}"><label style="font-size:11px;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:.08em;display:block;margin-bottom:5px">Bio</label><input class="edit-field-input" id="ep-bio" value="${UI.esc(u.bio||'')}" placeholder="Something about you…"><div class="edit-field-actions"><button class="edit-field-btn cancel" onclick="this.closest('.edit-field-overlay').remove()">Cancel</button><button class="edit-field-btn save" onclick="App.saveProfileEdit()">Save</button></div></div>`;
    overlay.onclick=e=>{if(e.target===overlay)overlay.remove();};document.body.appendChild(overlay);
    document.getElementById('ep-file').addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;try{const d=await API.upload(f);document.getElementById('ep-avatar').innerHTML=`<img src="${d.url}" style="width:100%;height:100%;object-fit:cover">`;currentUser.avatar=d.url;UI.toast('Photo updated ✓');}catch{UI.toast('Upload failed');}});
  }
  ...
  async function saveProfileEdit(){
    ...
    try{const d=await API.updateProfile({display_name:name,bio,avatar:currentUser.avatar||''});currentUser=d.user;updateMenuProfile();document.querySelector('.edit-field-overlay')?.remove();showSettings();UI.toast('Profile saved ✓');}
    catch(err){UI.toast('Failed: '+err.message);}
  }

  function toggleThemeSettings(){const isDark=document.documentElement.getAttribute('data-theme')==='dark';UI.setTheme(!isDark);setTimeout(showSettings,50);}

  function showEditField(field,label,currentVal){
    const overlay=document.createElement('div');overlay.className='edit-field-overlay';
    overlay.innerHTML=`<div class="edit-field-box"><div class="edit-field-title">Edit ${label}</div><input class="edit-field-input" id="sef-input" value="${UI.esc(currentVal)}" placeholder="${label}…"><div class="edit-field-actions"><button class="edit-field-btn cancel" onclick="this.closest('.edit-field-overlay').remove()">Cancel</button><button class="edit-field-btn save" onclick="App.saveFieldEdit('${field}')">Save</button></div></div>`;
    overlay.onclick=e=>{if(e.target===overlay)overlay.remove();};document.body.appendChild(overlay);
  ...
  function closeChat(){
    document.getElementById('chat-view').classList.add('hidden');document.getElementById('splash').classList.remove('hidden');
  ...
  async function deleteChat(){
    ...
    try{if(isGroup)await API.leaveGroup(activeChatId).catch(()=>{});else await API.deleteChat(activeChatId).catch(()=>{});}catch{}
    chats=chats.filter(c=>c.id!==activeChatId);closeChat();renderChatList();UI.toast(isGroup?'Left group':'Chat deleted');
  }
  ...
  return{
    ...
    toggleThemeSettings,showEditField,saveFieldEdit,showContacts,openMenu,closeMenu,
